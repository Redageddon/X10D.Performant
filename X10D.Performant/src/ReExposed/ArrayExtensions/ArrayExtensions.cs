...
public static class ArrayExtensions
{
    ...
    public static ReadOnlyCollection<T> AsReadOnly<T>(this T[] values) => Array.AsReadOnly(values);
    ...
    public static int BinarySearch<T>(this T[] values, T? value, int index = 0, int? length = null, IComparer<T?>? comparer = null) =>
        Array.BinarySearch(values, index, length ?? values.Length - index, value, comparer);
    ...
    public static TTo[] ConvertAll<TFrom, TTo>(this TFrom[] values, Converter<TFrom, TTo> converter) => Array.ConvertAll(values, converter);
    ...
    public static T? Find<T>(this T[] values, Predicate<T> predicate) => Array.Find(values, predicate);

    /// <inheritdoc cref="Array.FindAll{T}(T[],Predicate{T})"/>
    public static T[] FindAll<T>(this T[] values, Predicate<T> match) => Array.FindAll(values, match);
    ...
    public static int FindIndex<T>(this T[] values, Predicate<T> match, int index = 0, int? count = null) =>
        Array.FindIndex(values, index, count ?? values.Length - index, match);

    /// <inheritdoc cref="Array.FindLast{T}(T[],Predicate{T})"/>
    public static T? FindLast<T>(this T[] values, Predicate<T> match) => Array.FindLast(values, match);
    ...
    public static int IndexOf<T>(this T[] values, T value, int index = 0, int? count = null) =>
        Array.IndexOf(values, value, index, count ?? values.Length - index);
    ...
    public static void Sort<T>(this T[] values, Comparison<T> comparison) => Array.Sort(values, comparison);

    /// <inheritdoc cref="Array.TrueForAll{T}(T[],Predicate{T})"/>
    public static bool TrueForAll<T>(this T[] values, Predicate<T> match) => Array.TrueForAll(values, match);
}